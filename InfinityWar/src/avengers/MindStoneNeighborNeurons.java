...
public class MindStoneNeighborNeurons {
    ...
    public static void main (String [] args) {
        ...
        ArrayList<String> neuronSet = new ArrayList<>();
        String mindStoneLocation = "OOGABOOGA";

        for (int i = 0; i < neurons; i++) {

            neuronSet.add(StdIn.readString());
        }

        int linkCount = StdIn.readInt();
        ...
        for (int i = 0; i < neurons; i++) {
            ...
            if (!network.containsKey(neuronSet.get(i))) {

                mindStoneLocation = neuronSet.get(i);
                mindStoneIndex = i;
            }
        }

        neuronSet.remove(mindStoneIndex);

        for (int i = 0; i < neurons - 1; i++) {

            if(network.get(neuronSet.get(i)).equals(mindStoneLocation)) {

                StdOut.println(neuronSet.get(i));
            }
        }
    }
}
